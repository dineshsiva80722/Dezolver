...
router.post(
  ...
  [
    ...
    body('solution_approach')
      .notEmpty()
      .isString()
      .trim()
      .withMessage('Solution approach is required'),
    body('time_complexity').notEmpty().isString().trim().withMessage('Time complexity is required'),
    body('space_complexity')
      .notEmpty()
      .isString()
      .trim()
      .withMessage('Space complexity is required'),
    body('code_snippets').optional().isArray().withMessage('Code snippets must be an array'),
    body('code_snippets.*.language')
      .isString()
      .withMessage('Language is required for each code snippet'),
    body('code_snippets.*.code').isString().withMessage('Code is required for each snippet')
  ],
  validate,
...
router.post(
  ...
  [
    ...
    body('language').notEmpty().isString().trim().withMessage('Language is required'),
    body('code')
      .notEmpty()
      .isString()
      .trim()
