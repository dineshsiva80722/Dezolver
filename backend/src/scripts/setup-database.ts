...
async function setupDatabase() {
  try {
    ...
    const managerSqlPath = path.join(__dirname, '../database/migrations/add_manager_simple.sql');
    if (fs.existsSync(managerSqlPath)) {
      const sql = fs.readFileSync(managerSqlPath, 'utf8');
      await AppDataSource.query(sql);
      logger.info('Manager hierarchy SQL applied');
    }
