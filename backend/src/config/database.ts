...
import { PayrollCycle } from '../models/PayrollCycle.entity';
...
const pgConfig = {
  host: parsedDbUrl ? parsedDbUrl.hostname : (process.env.DB_HOST || 'localhost'),
  port: parsedDbUrl ? parseInt(parsedDbUrl.port || '5432') : parseInt(process.env.DB_PORT || '5432'),
  database: parsedDbUrl ? (parsedDbUrl.pathname.replace('/', '') || process.env.DB_NAME || 'techfolks_db') : (process.env.DB_NAME || 'techfolks_db'),
  user: parsedDbUrl ? (parsedDbUrl.username || process.env.DB_USER || 'postgres') : (process.env.DB_USER || 'postgres'),
  password: parsedDbUrl ? (parsedDbUrl.password || process.env.DB_PASSWORD) : process.env.DB_PASSWORD,
...
export const AppDataSource = new DataSource({
  ...
  password: pgConfig.password || 'postgres',
