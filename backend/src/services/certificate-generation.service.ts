...
import {
  ...
  CertificateStatus
} from '../models/Certificate.entity';
...
export class CertificateGenerationService {
  ...
  async generateCertificate(data: GenerateCertificateDTO): Promise<Certificate> {
    ...
    try {
      const { pdfUrl, imageUrl, qrCodeUrl } = await this.generateCertificatePDF(
        savedCertificate,
        template,
        learner
      );
  ...
  async batchGenerateCertificates(certificates: GenerateCertificateDTO[]): Promise<Certificate[]> {
    ...
    for (const certificateData of certificates) {
      try {
        ...
        console.error(
          `Failed to generate certificate for learner ${certificateData.learner_id}:`,
          error
        );
        throw error;
      }
