...
export class OrganizationService {
  ...
  async createOrganization(data: CreateOrganizationDTO): Promise<{
    ...
    try {
      ...
      const hashedPassword = await import('bcryptjs').then(bcrypt => bcrypt.hash(data.manager_user.password, 10));
  ...
  private calculatePricePerUser(plan: OrganizationPlan): number {
    switch (plan) {
      case OrganizationPlan.STARTER: return 120; // ₹120 per user
      case OrganizationPlan.PROFESSIONAL: return 100; // ₹100 per user
      case OrganizationPlan.ENTERPRISE: return 80; // ₹80 per user
      case OrganizationPlan.UNLIMITED: return 0; // Fixed price
      default: return 120;
    }
  }
  ...
  async checkUserLimit(organizationId: string): Promise<{
    ...
    const remainingSlots = organization.user_limit === -1 
      ? -1 
      : Math.max(0, organization.user_limit - currentUsers);
  ...
  async getOrganizationStats(organizationId: string): Promise<{
    ...
    const utilizationPercentage = organization.user_limit === -1 
      ? 0 
  ...
  async upgradePlan(
    ...
    if (planLimits.users !== -1 && organization.current_users > planLimits.users) {
      throw new Error(`Cannot downgrade: You have ${organization.current_users} users but the ${newPlan} plan only allows ${planLimits.users} users.`);
    }
  ...
  private calculateTotalPrice(plan: OrganizationPlan, userCount: number, billingCycle: string): number {
    const basePrice = this.planLimits[plan].price;
    const pricePerUser = this.calculatePricePerUser(plan);
    
    let totalMonthly = basePrice + (pricePerUser * userCount);
  ...
  async getPlatformStats(): Promise<{
    ...
    const totalRevenue = subscriptions.reduce((sum, sub) => sum + parseFloat(sub.total_amount.toString()), 0);
    const totalUsers = await this.userRepository.count();

    const planDistribution = {
      [OrganizationPlan.STARTER]: organizations.filter(org => org.plan === OrganizationPlan.STARTER).length,
      [OrganizationPlan.PROFESSIONAL]: organizations.filter(org => org.plan === OrganizationPlan.PROFESSIONAL).length,
      [OrganizationPlan.ENTERPRISE]: organizations.filter(org => org.plan === OrganizationPlan.ENTERPRISE).length,
      [OrganizationPlan.UNLIMITED]: organizations.filter(org => org.plan === OrganizationPlan.UNLIMITED).length,
    };

    return {
      totalOrganizations: organizations.length,
      activeOrganizations: organizations.filter(org => org.status === OrganizationStatus.ACTIVE).length,
      totalRevenue,
      totalUsers,
      planDistribution,
    };
