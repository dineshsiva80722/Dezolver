...
export class SalarySlipService {
  ...
  private addEmployeeDetails(doc: PDFKit.PDFDocument, payrollRecord: PayrollRecord): void {
    ...
    doc.fontSize(14).font('Helvetica-Bold').text('Employee Details', 50, yStart);
    ...
    doc
      .text(
        `Pay Period: ${new Date(payrollRecord.pay_period_start).toLocaleDateString()} to ${new Date(payrollRecord.pay_period_end).toLocaleDateString()}`,
        300,
        yStart + 25
      )
  ...
  private addPayrollSummary(doc: PDFKit.PDFDocument, payrollRecord: PayrollRecord): void {
    ...
    doc.fontSize(12).font('Helvetica-Bold').text('PAYROLL SUMMARY', 50, yStart);

    doc.rect(50, yStart + 20, 445, 60).stroke();
    ...
    doc
      ...
      .font('Helvetica')
      .text(
        `₹${payrollRecord.basic_salary.toLocaleString('en-IN', { minimumFractionDigits: 2 })}`,
        60,
        yStart + 50
      )
      .text(
        `₹${payrollRecord.gross_salary.toLocaleString('en-IN', { minimumFractionDigits: 2 })}`,
        200,
        yStart + 50
      )
      .text(
        `₹${payrollRecord.total_deductions.toLocaleString('en-IN', { minimumFractionDigits: 2 })}`,
        320,
        yStart + 50
      );
    ...
    doc
      .font('Helvetica-Bold')
      .text(
        `₹${payrollRecord.net_salary.toLocaleString('en-IN', { minimumFractionDigits: 2 })}`,
        440,
        yStart + 50
      );
  }
  ...
  private addEarningsSection(doc: PDFKit.PDFDocument, payrollRecord: PayrollRecord): void {
    ...
    doc.fontSize(12).font('Helvetica-Bold').text('EARNINGS', 50, currentY);

    currentY += 25;

    doc.rect(50, currentY, 220, 20).stroke();
    ...
    earningKeys.forEach((key) => {
      if (key === 'custom_allowances' && Array.isArray(earnings.custom_allowances)) {
        earnings.custom_allowances.forEach((allowance: any) => {
          doc
            .text(allowance.name || key, 60, currentY + 5)
            .text(
              allowance.amount.toLocaleString('en-IN', { minimumFractionDigits: 2 }),
              210,
              currentY + 5
            );
          currentY += 15;
        ...
        doc
          .text(displayName, 60, currentY + 5)
          .text(
            earnings[key].toLocaleString('en-IN', { minimumFractionDigits: 2 }),
            210,
            currentY + 5
          );
        currentY += 15;
    ...
    doc.rect(50, yStart + 20, 220, currentY - yStart - 20).stroke();
  ...
  private addDeductionsSection(doc: PDFKit.PDFDocument, payrollRecord: PayrollRecord): void {
    ...
    doc.fontSize(12).font('Helvetica-Bold').text('DEDUCTIONS', 325, currentY);

    currentY += 25;

    doc.rect(325, currentY, 220, 20).stroke();
    ...
    deductionKeys.forEach((key) => {
      if (key === 'custom_deductions' && Array.isArray(deductions.custom_deductions)) {
        ...
        doc
          ...
          .text(
            deductions[key].toLocaleString('en-IN', { minimumFractionDigits: 2 }),
            485,
            currentY + 5
          );
        currentY += 15;
    ...
    doc.rect(325, yStart + 20, 220, currentY - yStart - 20).stroke();
  ...
  private addNetPaySection(doc: PDFKit.PDFDocument, payrollRecord: PayrollRecord): void {
    ...
    doc.rect(50, yPos, 495, 40).fill('#f8f9fa').stroke();
